...
bool __fastcall Hooked_CreateMove(PVOID ClientMode, int edx, float input_sample_frametime, CUserCmd* pCommand)
{
	...
	try
	{
		...
		if (pLocal->IsAlive())
		{
			...
			{
				if (!gCvars.misc_fakelag_onattack && (pCommand->buttons & IN_ATTACK))
					return false;
		...
		gMisc.Run(pLocal, pCommand); //Misc Features
...
void __stdcall Hooked_DrawModelExecute(void *state, ModelRenderInfo_t &pInfo, matrix3x4_t *pCustomBoneToWorld)
{
	...
	if (strstr(pszModelName, "models/player") && gCvars.esp_chams > 0) 
	{
	...
	if (GAME_TF2)
	{
		if (gCvars.misc_hands == 1 && strstr(pszModelName, "arms")) {
			gInts.RenderView->SetBlend(0);
			gInts.MdlRender->DrawModelExecute(state, pInfo, pCustomBoneToWorld);
		}
		else if (gCvars.misc_hands == 2 && strstr(pszModelName, "arms")) {
...
int __fastcall Hooked_KeyEvent(PVOID CHLClient, int edx, int eventcode, int keynum, const char *currentBinding)
{
	if (eventcode == 1)
	{
		...
		{
			//These are only the mouse keys, keyboard keys are done in Panels.cpp
			if (keynum == 112) // Up
			{
